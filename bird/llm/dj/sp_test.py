...
from utils import set_sp_env, generate_combined_prompts_one
...
# def gpt_chat_request(engine, prompt, max_tokens, temperature, stop):
...
def gpt_completion_request(engine, prompt, max_tokens, temperature, stop):
    print('requesting gpt...', end='', flush=True)
    llm = OpenAI(model='vicuna-13b-v1.1', temperature=temperature, max_tokens=max_tokens)
    try:
        result = llm(prompt, stop=stop)
    except Exception as e:
        result = 'error:{}'.format(e)
    print('gpt response received')
    return result
...
def get_more_questions(engine, question, db_path, knowledge, use_knowledge):
    ...
    """
    #print(prompt)
    plain_result = gpt_completion_request(engine=engine, prompt=prompt, max_tokens=256, temperature=0, stop=['--', '\n\n', ';', '#', '```'])
    try:
        ret = strip(plain_result).strip()
    except Exception as e:
        print(plain_result)
...
def get_answer(engine, question, db_path, knowledge, use_knowledge, fewshots=None):
    ...
    plain_result = gpt_completion_request(engine=engine, prompt=prompt + '\nSELECT ', max_tokens=256, temperature=0, stop=['--', '\n\n', ';', '#', '```'])
    try:
        ret = 'SELECT\n' + strip(plain_result)
    except Exception as e:
        print(plain_result)
